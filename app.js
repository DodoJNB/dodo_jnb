...
/* ── Licznik odwiedzin (dzisiaj) ──
   Korzysta z darmowego, publicznego countapi (bez własnego backendu).
   Klucz = nazwa strony + dzisiejsza data (Europe/Warsaw), więc o 0:00
   zaczyna się nowy klucz i licznik sam "zeruje się" na nowy dzień.
   sessionStorage pilnuje, żeby jedna sesja nie podbijała licznika
...
(async () => {
  ...
  const counterKey = `dodo_jnb_visits_${todayWarsaw}`;
  const alreadyCounted = sessionStorage.getItem('dodo_counted_' + todayWarsaw);
  ...
  try {
    ...
    const data = await res.json();
    if (!alreadyCounted) sessionStorage.setItem('dodo_counted_' + todayWarsaw, '1');
    if (data && data.value != null) {
      el.querySelector('.visit-num').textContent = data.value;
    } else {
